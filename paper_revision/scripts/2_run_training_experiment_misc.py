...
from samplers import PairPositiveNegativeSamplerMiscl
...
parser.add_argument("--miscl_perc", type=int, default=10, help="Percentage of misclassified pairs")
...
input_csv = os.path.join(
    ...
    "..",
    "data",
    f"ml_datasets_pairs_{miscl_perc}",
...
if not os.path.exists(os.path.join(model_base, f"models_pairs_{miscl_perc}")):
    os.mkdir(os.path.join(model_base, f"models_pairs_{miscl_perc}"))

model_folder = os.path.join(model_base, f"models_pairs_{miscl_perc}", model_name)
...
for i in range(n_folds):
    ...
    ds = PairPositiveNegativeSamplerMiscl(df, miscl_perc, negative_ratio).sample()
