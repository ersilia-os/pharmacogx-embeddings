...
def hap_to_var(df):
    df2 = pd.read_csv(os.path.join(processed_folder, "haplotype.csv"))
    df2["variant"] = df2.apply(lambda row: row['rsID'] if pd.notnull(row['rsID']) else str(row["gene"] + " " + row['start']) if pd.notnull(row['start']) else None, axis=1)
    R = []
    for r in df2.values:
        hid = r[0]
        haplotype = r[1]
        ...
        for r_ in df.values:
            ...
            var_  = r_[2]
            if var == var_:
                r = [hid,haplotype,haplotype_number,gid,gene,rsID,start,protein,nc,ng,var,vid]
                R += [r]
    cols = ["hid", "haplotype", "haplotype_number", "gid", "gene", "rsID", "start", "protein", "nc", "ng", "variant", "vid"]
    data = pd.DataFrame(R, columns=cols)
...
def add_vars(df):
    df2 = pd.read_csv(os.path.join(processed_folder, "haplotype.csv"))
    df2["variant"] = df2.apply(lambda row: row['rsID'].strip() if pd.notnull(row['rsID'])
                           else row['NC'].strip() if pd.notnull(row['NC'])
                           else str(row["gene"] + " " + row['start']).strip() if pd.notnull(row['gene']) and pd.notnull(row['start'])
                           else None, axis=1)
    df2["variant"] = df2["variant"].apply(lambda var: var.rstrip("*") if  pd.notnull(var) else var)
    df2.loc[df2["variant"] == "NAT2 Δ859", "variant"] = "NAT2 859Del"
    vid_mapping = df.groupby('variant')['vid'].first().reset_index() #get unique vid-variant
...
def deconv_doubles(df):
    ...
    vid_mapping = df2.groupby('variant')['vid'].first().reset_index() #get unique vid-variant
