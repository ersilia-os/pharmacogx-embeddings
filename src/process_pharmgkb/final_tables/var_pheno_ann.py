...
df = pd.read_csv(os.path.join(processed_folder, "var_pheno_ann.csv"))
data = hap_to_var(df)
data = add_gid_ensembl_id(data)
data = add_cid_smiles(data)
data = clean_haps(data)
data = add_empty_cols(data)
data = data[["cid", "chemical", "smiles", "gid", "gene", "ensembl_id", "vid", "variant","evidence", "significance", 
             "phenotype","did", "disease","biogroup", "caid", "vaid"]]
data.to_csv(os.path.join(final_folder, "var_pheno_ann.csv"), index=False)