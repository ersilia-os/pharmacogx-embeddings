...
def get_raw_files():
    r = RawData()
    var_pheno_ann = r.var_pheno_ann
    return var_pheno_ann

def deconv_genomic_var():
    c = CsvCleaner()
    var_pheno_ann = get_raw_files()
...
def create_table():
    data  = deconv_pheno()
    data.to_csv(os.path.join(processed_folder, "pgx_relation_int", "var_pheno_ann_int.csv"), index=False)
