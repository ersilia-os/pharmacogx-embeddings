...
class BaseTrimodalModel(object):
    def __init__(
        ...
        self.n_components_single_reducer_3 = n_components_single_reducer_3
    ...
    def fit(self, X1, X2, X3, y):
        ...
        self.single_reducer_3 = PCA(
            n_components=self.n_components_single_reducer_3, svd_solver="randomized"
        )
        ...
        print("... single reducer 3")
        X2 = self.single_reducer_3.fit_transform(X3)
    ...
    def predict(self, X1, X2, X3):
        ...
        X3 = self.single_reducer_2.transform(X3)
        X = np.hstack([X1, X2, X3])
        X = self.stacked_reducer.transform(X)
        return self.model.predict(X)

    def predict_proba(self, X1, X2, X3):
        X1 = self.single_reducer_1.transform(X1)
        X2 = self.single_reducer_2.transform(X2)
        X3 = self.single_reducer_2.transform(X3)
        X = np.hstack([X1, X2, X3])
...
class TrimodalStackedModel(object):
    ...
    def fit(self, df):
        triplets = np.array(df[["inchikey", "uniprot_ac", "variant"]].values)
        y = np.array(df["y"])
        A = []
        B = []
        C = []
        ...
        self.model.fit(A, B, C, y)
        self._is_fitted = True

    def predict(self, df):
        assert self._is_fitted
        triplets = np.array(df[["inchikey", "uniprot_ac", "variant"]])
        A = []
        B = []
        C = []
        idxs = []
        ...
        B = np.array(B)
        C = np.array(C)
        y_ = self.model.predict_proba(A, B, C)[:, 1]
...
def load_ensemble_trimodal_stacked_model(model_folder):
...
class EnsembleTrimodalStackedModel(object):
    ...
    def get_name(self, cn, pn, vn):
        if type(pn) is str:
            return cn + "---" + pn + "---" + vn
        else:
            return cn + "---" + "--".join(list(pn)) + "---" + vn
    ...
    def fit(self, df):
        for emb_name_A in self.emb_name_list_A:
            for emb_name_B in self.emb_name_list_B:
                for emb_name_C in self.emb_name_list_C:
                    name = self.get_name(emb_name_A, emb_name_B, emb_name_C)
                    model = TrimodalStackedModel(emb_name_A, emb_name_B, emb_name_C)
                    print("Fitting:", name)
                    model.fit(df)
