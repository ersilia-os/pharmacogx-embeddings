...
class BiotequeCompoundEmbedding(object):
    def __init__(self, metapath="CPD-int-GEN", dataset="string"):
        self.metapath = metapath
        self.dataset = dataset
        self.h5_file = os.path.join(bioteque_path, metapath, dataset, "CPD_emb.h5")
        self.ids_file = os.path.join(bioteque_path, metapath, dataset, "CPD_ids.txt")
    ...
    def available(self):
        ...
        for fn in os.listdir(bioteque_path):
            ...
            for fm in os.listdir(os.path.join(bioteque_path, fn)):
                if not os.path.exists(
                    os.path.join(bioteque_path, fn, fm, "CPD_emb.h5")
                ):
                    continue
                R += [(fn, fm)]
        df = pd.DataFrame(R, columns=["metapath", "dataset"])
