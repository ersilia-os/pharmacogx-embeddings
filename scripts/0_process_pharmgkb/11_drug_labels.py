...
def deconv_hap_variant(df):
    c = CsvCleaner()
    vars = pd.read_csv(os.path.join(processed_folder, "5_variant_complete.csv"))
    haps = pd.read_csv(os.path.join(processed_folder, "2_haplotype.csv"))
...
if __name__ == "__main__":
    ...
    df = clean_haps(df)
    df.to_csv(os.path.join(processed_folder, "11_drug_labels.csv"), index=False)
